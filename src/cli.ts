...
// This interface defines the expected structure of the user's ai-context-builder.config.ts
 ...
 * @param configPath Path to the ai-context-builder.config.ts file.
...
async function generateContextFile(config: ResolvedAiContextConfig): Promise<void> {
    ...
    finalMarkdownContent += `<!-- Context generated by ai-context-builder on ${new Date().toISOString()} -->\n`
...
async function main() {
    ...
    const defaultConfigFileName = 'ai-context-builder.config.ts'
    ...
    program
        ...
        .addOption(
            new Option('-c, --config <path>', 'Path to the configuration file (e.g., ai-context-builder.config.ts)').default(defaultConfigFileName),
        )
        .addOption(
    ...
    console.log('🚀 ai-context-builder CLI starting...')
